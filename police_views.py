...
class view_evidence(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        m = Complaint.objects.filter(police__login=self.request.user.id,status='approved',status1='fir_registered')
        context['m'] = m
...
class evidence_details_view(TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        id=self.request.GET['id']
        m = evidence.objects.get(ledger_id=id)
...
class editCriminals(LoginRequiredMixin,TemplateView):
    template_name = 'police_staff/edit_criminal.html'
    login_url = '/'

    def get_context_data(self, **kwargs):
        context = super(editCriminals, self).get_context_data(**kwargs)
        id = self.request.GET['id']

        cri = Criminals.objects.get(id=id)
    ...
    def post(self, request, *args, **kwargs):
        ...
        messages = "Added Successfully"
        return render(request,'police_staff/edit_criminal.html',{'message':messages})