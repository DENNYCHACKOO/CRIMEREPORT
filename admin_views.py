...
class editStation(LoginRequiredMixin,TemplateView):
    template_name = 'admin/editstation.html'
    login_url = '/'
    
    def get_context_data(self,**kwargs):
        id = self.request.GET['ps_id']

        context = super(editStation,self).get_context_data(**kwargs)

        single_view = PoliceStation.objects.get(id=id)

        context['p'] = single_view
        return context
    ...
    def post(self, request, *args, **kwargs):
        ...
        s = PoliceStation.objects.get(id=id)
        s.station_name = station
        s.save()

        messages = "Added Successfully"
        return render(request,'admin/editstation.html',{'message':messages})
...
class Profile_details(TemplateView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        range = request.POST['rangeee']
        range11 = request.POST['range']

        user = request.POST['user']
        officer=request.POST['officer']

        ps = User.objects.get(id=user)
        pl = Officer_Reg.objects.get(id=officer)

        ps.first_name = name
        pl.designation = desig
        pl.station_id = range
        pl.save()
        ps.save()

        messages.success(request, 'Update successfull')
        return redirect('admin:view_officer')
